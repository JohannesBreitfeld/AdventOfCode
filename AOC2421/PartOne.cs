...
internal class PartOne
{
    ...
    private Dictionary<(char from, char to), List<string>> shortestPath = new();
    ...
    private string GetShortestSequence(string code, int layer)
    {
        ...
        for (int i = 0; i < code.Length; i++)
        {
            ...
            var shortestPaths = shortestPath[(previous, current)];
    ...
    private void CacheAllShortestSequences(char[,] keypad)
    {
        foreach (var character in keypad)
        {
            ...
            {
                shortestPath[(character, otherCharacter)] = new List<string>();

                if (character == otherCharacter)
                {
                    shortestPath[(character, otherCharacter)].Add("A");
                }
                else
                {
    ...
    private void CacheShortestSequence(char from, char to, char[,] keypad)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            {
                shortestPath[(from, to)].Add(sequence + "A");
                continue;
            }
